...
The workflow file is included under `.github/workflows/ci.yml` inside this project folder, where it is a template only — GitHub discovers workflows exclusively in the repository-level `.github/workflows` directory. To activate it in a portfolio monorepo, copy it to the repository-level `.github/workflows/ci.yml` and adjust the `working-directory` and artifact `path` values to match the actual repo layout (in this repository: `Projects/32-ci-pipeline-java-app`).
