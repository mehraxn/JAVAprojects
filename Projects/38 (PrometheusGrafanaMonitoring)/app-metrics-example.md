...
| Metric | Type | Meaning |
|---|---|---|
| `http_requests_total` | Counter | Requests grouped by bounded method, route, and status labels |
...
The `route` label takes only five stable values — `/`, `/health`, `/metrics`,
`/work`, and `not_found` (used for every unknown path). Do not place user IDs,
raw URLs, request IDs, email addresses, or other unbounded values in metric
labels; each unique label combination creates another time series.
...
```text
up{job="java-app"}
sum by (route) (rate(http_requests_total{job="java-app"}[5m]))
...
The metric contract, scrape behavior, PromQL queries, and provisioning were
verified against the running Compose stack on 2026-07-10 — see
[TEST_RESULTS.md](TEST_RESULTS.md). Alert rules were loaded and evaluating;
no alert was driven all the way to firing.
