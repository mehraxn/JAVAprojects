...
// R6
	...
	public boolean deleteElement(String name) {
		...
		if (target == null) {
			return true; // Element not found, technically not deleted but...
		}

		// (R6) If Split or Multisplit with > 1 output, no operation.
		if (target instanceof Split) { // This also catches Multisplit
		...
		// Find the upstream element
		Element upstream = findUpstreamElement(target);
		
		// Find the downstream element (only for 0 or 1 output)
		...
		// Rewire
		if (upstream != null) {
			// This is tricky. We need to find *which* output of upstream
			// was connected to target.
		...
		if (targetIndex != -1) {
			...
			elements[elementCount - 1] = null; // Clear last
			elementCount--;
		}
