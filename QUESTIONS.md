# 📘 Object Oriented Programming – Quiz (10 Questions + Answers)

This README contains the 10 questions extracted from the provided images, **with the correct answers**.
...
✅ **Answer:** **None of the listed options is correct as written.**
...
> Note: options using `c++` return the *old* value (and also fail when `c` is `null`), so they do not correctly update the stored frequency.
